...
{
	ESimOne_VehicleEventInfo_V2XWarningInfoType_FCW = 0,		///<FCW: Forward Collision Warning
	ESimOne_VehicleEventInfo_V2XWarningInfoType_ICW = 1,		///<ICW: Intersection Collision Warning
	ESimOne_VehicleEventInfo_V2XWarningInfoType_LTA = 2,		///<LTA: Left Turn Assist
	ESimOne_VehicleEventInfo_V2XWarningInfoType_BSW_LCW = 3,	///<BSW/LCW: Blind Spot Warning/Lane Change Warning
	ESimOne_VehicleEventInfo_V2XWarningInfoType_DNPW = 4,		///<DNPW: Do Not Pass Warning
	ESimOne_VehicleEventInfo_V2XWarningInfoType_EBW = 5,		///<EBW: Emergency Brake Warning
	ESimOne_VehicleEventInfo_V2XWarningInfoType_AVW = 6,		///<AVW: Abnormal Vehicle Warning
	ESimOne_VehicleEventInfo_V2XWarningInfoType_CLW = 7,		///<CLWControl Loss Warning
	ESimOne_VehicleEventInfo_V2XWarningInfoType_HLW = 8,		///<HLW: Hazardous Location Warning
	ESimOne_VehicleEventInfo_V2XWarningInfoType_SLW = 9,		///<SLW: Speed Limit Warning
	ESimOne_VehicleEventInfo_V2XWarningInfoType_RLVW = 10,		///<RLVW: Red Light Violation Warning
	ESimOne_VehicleEventInfo_V2XWarningInfoType_VRUCW = 11,		///<VRUCW: Vulnerable Road User Collision Warning
	ESimOne_VehicleEventInfo_V2XWarningInfoType_GLOSA = 12,	    ///<GLOSA: Green Light Optimal Speed Advisory
	ESimOne_VehicleEventInfo_V2XWarningInfoType_IVS = 13,       ///<IVS: In-Vehicle Signage
	ESimOne_VehicleEventInfo_V2XWarningInfoType_TJW = 14,		///<TJW: Traffic Jam Warning
	ESimOne_VehicleEventInfo_V2XWarningInfoType_EVW = 15,		///<EVW: Emergency Vehicle Warning
...
{
	char mainVehicleId[MAX_MAINVEHICLE_NAME_LEN]; // main vehicle ID
	float axisDistance;   //vehicle axis distance unit: cm
	//float axisHeight;     //vehicle axie height  unit: cm
	float cmFrontAxisOffset;  // vehicle CM offset to front axis   unit: cm
	float cmHeight;       // the height of vehicle CM   unit: cm
	float cmYOffset;      // vehicle CM offset to Y shaft unit: cm
	float chassisHeight;  // the height of vehicle chassis   unit: cm
	float chassisWidth;   // the width of vehicle chassis unit: cm
	float frontLeftWheelOffset;   // unit: cm
	float frontRightWheelOffset;  // unit: cm
	float frontToFrontAxis;       // unit: cm
	float rearAxisToRear;   // unit: cm
	float RearLeftWheelOffset;   // unit: cm
	float RearRightWheelOffset;   // unit: cm
...
{
	int markerSize; // Marker size
	SimOne_Data_Marker_Entry markers[SOSM_MARKER_SIZE_MAX]; // Marker, 100 max
};
...
{
	float accelX; // Acceleration X on IMU coordinate system ( m/s^2 )
	float accelY; // Acceleration Y on IMU coordinate system ( m/s^2 )
	float accelZ; // Acceleration Z on IMU coordinate system ( m/s^2 )

	float velX; // Velocity X on Opendrive ( m/s )
	float velY; // Velocity Y on Opendrive ( m/s )
	float velZ; // Velocity Z on Opendrive ( m/s )

	float angVelX; // Angular Velocity Roll on IMU coordinate system ( rad/s )
	float angVelY; // Angular Velocity Pitch on IMU coordinate system ( rad/s )
	float angVelZ; // Angular Velocity Yaw on IMU coordinate system ( rad/s )

	float rotX; // Rotation Roll on Opendrive ( rad )
	float rotY; // Rotation Pitch on Opendrive ( rad )
	float rotZ; // Rotation Yaw on Opendrive ( rad )
};
...
struct SimOne_Data_Gps : public SimOne_Data
{
	...
	float angVelX; // MainVehicle Angular Velocity X on Opendrive (by radian)
	float angVelY; // MainVehicle Angular Velocity Y on Opendrive (by radian)
	float angVelZ; // MainVehicle Angular Velocity Z on Opendrive (by radian)
	...
	bool isGPSLost;
	char mainCarName[SOSM_ACTORNAME_LENGT];   // encoding: utf-8
...
{
	float timeOfDay=1200;		// [0, 2400], (HHmm). Time of day in the scence.
	// Simulate Reality
	bool simulateReality=true;	// Whether to use the location & date & time attributes to determine the position of the sun, moon and stars.
	float longitude= 121.4737;	// [-180, 180]. When simulateReality is true, it is used as the weather simulation location.
	float latitude = 31.2304;	// [-90, 90]. When simulateReality is true, it is used as the weather simulation location.
	int timeZone = 8;			// [-12, 12]. When simulateReality is true, it is used as the weather simulation timeZone.
	int date = 20230921;		// [18000101, 21001231], (yyyyMMdd). When simulateReality is true, it is used as the weather simulation date.
	// Sun
	float sunIntensity = 5;		// [0, 30]. The intensity of Sun light.
	float sunAzimuth = 0;		// [0, 360]. Measure the angle clockwise from true north (0) along the horizon to determine the direction 
								//			 in which the sun rises or sets. Invalid when simulateReality is true.
	float sunInclination = 30;	// [-85, 85]. The angle between the incident direction of sunlight and the zenith direction. 
								//			 Invalid when simulateReality is true.
	float sunSize = 1;			// [0, 5]. The size of the sun in the scene.
	// Moon
	float moonIntensity = 0.12;	// [0, 10]. The intensity of moon light.
	float moonAzimuth = 15;		// [0, 360]. Same as attribute sunAzimuth.
	float moonInclination = 35;	// [-90, 90]. Same as attribute sunAzimuth.
	float moonSize = 0.7;		// [0, 12]. The size of the moon in the scene.
	float moonPhase = 0;		// [0, 30]. The phases of the moon in the scene.
	float moonColorA = 0.87;	// [0, 1]. The color of the moon in the scene A channel.
	int moonColorR = 178;		// [0, 255]. The color of the moon in the scene R channel.
	int moonColorG = 197;		// [0, 255]. The color of the moon in the scene G channel.
	int moonColorB = 239;		// [0, 255]. The color of the moon in the scene B channel.

	// Weather
	// Cloud
	float cloudDensity = 3;		// [0, 10]. Cloud coverage density in the scene.
	float cloudHeight = 1;		// [0, 2]. Cloud height coefficient in the scene.
	float cloudThickness = 1;	// [0.1, 2]. Cloud thickness coefficient in the scene.
	// Fog Wind
	float fogDensity = 3;		// [0, 10]. The density of fog in the scene
	float windDensity = 3;		// [0, 10]. The density of wind in the scene.
	float windDirection = 180;	// [0, 360]. The direction the wind is blowing from. North as 0 with clockwise.
	// Rain
	float rainDensity = 3;		// [0, 10]. The density of rain in the scene.
	float raindropDensity = 2000;	// [0, 20000]. The raindrop density coefficient in the scene.
	float raindropParticle = 1;	// [0, 3]. The raindrop particle size coefficient in the scene.
	// Snow
	float snowDensity = 3;		// [0, 10]. The density of snow in the scene.
	float snowflakeDensity = 2000;	// [0, 30000]. The snowflake density coefficient in the scene.
	float snowflakeParticle = 1;// [0, 3]. The snowflake particle size coefficient in the scene.
	// Thunder
	float thunderLightning = 0;	// [0, 10]. The density of lightning in the scene.
	float thunderInterval = 7;	// [0, 10]. Time interval between lightning occurrences.
	float thunderBrightness = 14;	// [1, 25]. Lightning brightness
	// Dust
	float dustDensity = 0;		// [0, 10]. The density of dust in the scene.
	float dustDensityCoefficient = 300;	// [0, 3000]. The dust density coefficient in the scene.
	float dustParticle = 0;		// [0, 3]. The dust particle size coefficient in the scene.

	// Material
	// Coverage
	float wetness = 0;			// [0, 1]. The degree of rain coverage in the scene.
	float snowCoverage = 0;		// [0, 1]. The degree of snow coverage in the scene.
	float dustCoverage = 0;		// [0, 1]. The degree of dust coverage in the scene
	// Damage & Dirty
	float roadDirtyLevel = 0;	// [0, 1]. The degree of dirtiness of the road in the scene.
	float lineDamageLevel = 0;	// [0, 1]. The degree of damage to the road lines in the scene
 	float signDirtyLevel = 0;	// [0, 1]. The degree of dirtiness of the sign in the scene.
	float signDamageLevel = 0;	// [0, 1]. The degree of damage to the sign in the scene
};
...
struct SimOne_Data_SensorDetections_Entry
{
	...
	char name[SOSM_ACTORNAME_LENGT]; //Detection Object name same with case edit
};
...
{
	int trafficlightNum = 0;	// Obstacle number
